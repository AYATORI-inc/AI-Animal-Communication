...
* **送信先URL:** `https://script.google.com/macros/s/XXXXX/exec` または `https://script.google.com/a/macros/ドメイン名/s/XXXXX/exec`（先生が配布）
* **メソッド:** `GET`（推奨・CORSエラーが発生しません）
* **リクエスト形式:** クエリパラメータ（URLに含める）

### 接続テスト（Hello World）

GASへの接続を確認するため、パラメータなしのGETリクエストを送信できます。
...
console.log(data.message); // "Hello World! GAS接続成功！"
```
...
クエリパラメータとして以下の項目を送信してください。

**必須項目:**
* `animalType` (文字列): 動物種類（例: "サル", "ライオン", "ペンギン", "カピバラ", "パンダ"）
...
**JavaScriptでの送信例:**
```javascript
const prompt = 'あなたは「サル」という動物です。性格は「元気で好奇心旺盛」。今「バナナ」を食べました。子供向けの口調で、60文字以内で感想を言ってください。';
...
* テキスト形式の場合、以下の項目を受け取る:
  * `prompt` (必須): テキストプロンプト
* パラメータなしのGETリクエストの場合、Hello Worldテストとして動作する。
...
2. `data.success`を確認して、成功か失敗かを判定
3. 成功の場合:
   * `data.message`でテキストを表示
   * `data.imageUrl`が存在する場合、`<img>`タグで画像を表示
   * `data.imageError`が存在する場合、エラーメッセージを表示
4. 失敗の場合:
   * `data.error`でエラーメッセージを表示
   * `data.statusCode`があれば、HTTPステータスコードを確認
   * `data.details`があれば、詳細なエラー情報を確認
...
## 5. POSTリクエスト（後方互換性・オプション）

POSTリクエストもサポートされていますが、GETリクエストの使用を推奨します。
...
const response = await fetch(gasUrl, {
  ...
  body: 'あなたは「サル」という動物です。...'
});
```

**注意:** POSTリクエストを使用する場合、CORSプリフライトリクエストが発生する可能性があります。GASの`doOptions`関数が適切に実装されている必要があります。
...
```javascript
async function callGAS(animalType, foodType, freeWord = null, likeLevel = null) {
  const gasUrl = 'https://script.google.com/macros/s/XXXXX/exec';
  
  // パラメータを構築
  const params = new URLSearchParams();
  params.append('animalType', animalType);
  ...
  if (likeLevel) {
    params.append('likeLevel', likeLevel);
  }
...
// 使用例
const result = await callGAS('サル', 'バナナ', null, '大好き');
if (result) {
  console.log('メッセージ:', result.message);
  if (result.imageUrl) {
    console.log('画像URL:', result.imageUrl);
  } else if (result.imageError) {
    console.error('画像生成エラー:', result.imageError);
  }
}
```
